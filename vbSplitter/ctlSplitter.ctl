...
Private mPanels(1) As Object 'Panel objects.
...
On Error Resume Next
    ...
    Set mPanels(0) = Nothing
    Set mPanels(1) = Nothing
End Sub

Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
On Error Resume Next
    'Read the property values.
    BorderStyle = PropBag.ReadProperty("BorderStyle")
    MaxSize = PropBag.ReadProperty("MaxSize")
    MinSize = PropBag.ReadProperty("MinSize")
    AutoResize = PropBag.ReadProperty("AutoResize")
    Orientation = PropBag.ReadProperty("Orientation")
    SelectedColor = PropBag.ReadProperty("SelectedColor")
    SplitterColor = PropBag.ReadProperty("SplitterColor")
    SplitterWidth = PropBag.ReadProperty("SplitterWidth")
End Sub
Public Sub UserControl_WriteProperties(PropBag As PropertyBag)
On Error Resume Next
    'Save the property values.
    PropBag.WriteProperty "BorderStyle", BorderStyle
    PropBag.WriteProperty "MaxSize", MaxSize
    PropBag.WriteProperty "MinSize", MinSize
    PropBag.WriteProperty "AutoResize", AutoResize
    PropBag.WriteProperty "Orientation", Orientation
    PropBag.WriteProperty "SelectedColor", SelectedColor
    PropBag.WriteProperty "SplitterColor", SplitterColor
    PropBag.WriteProperty "SplitterWidth", SplitterWidth
End Sub
'***********************************************************************************************
'Resize Events
'***********************************************************************************************
...
On Error Resume Next
    ...
    'If either panel object is not set, then exit.
    If mPanels(0) Is Nothing Or mPanels(1) Is Nothing Then Exit Sub
    ...
    Select Case mOrientation
        Case vbSplitHorizontal
            ...
            mPanels(0).Move 0, 0, sngWidth, mPanelSize
            ...
            mPanels(1).Move 0, sngTop, sngWidth, sngSize
        Case vbSplitVertical
            SplitterBar.Move mPanelSize, 0, mSplitterWidth, sngHeight

            'Resize the Child1 panel.
            mPanels(0).Move 0, 0, mPanelSize, sngHeight
            ...
            'Resize the Child2 panel.
            mPanels(1).Move sngLeft, 0, sngSize, sngHeight
    End Select
